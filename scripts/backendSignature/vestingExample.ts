...
import { rDataVestingAbi } from './RDataVestingAbi';
import { rDataAbi } from './RDataAbi';
import { log } from 'console';

const rDataAddress = '0x2bDE71a995616AEf0E4aF854A12b89a5E26e63D7';
const rDataVestingAddress = '0xD703b22ce502B39bF75ABbcEA621643b0Cf66bc0';
...
async function callClaimMethod(claimParams: ClaimParams) {
    ...
    const rDataVesting = new Contract(rDataVestingAddress, rDataVestingAbi, provider);

    const rData = new Contract(rDataAddress, rDataAbi, provider);

    log(claimParams);

    await rDataVesting.claim(
        claimParams.userId,
...
async function main() {
    ...
    await callClaimMethod(claimParams)  //call claim method from the Vesting smart contract
